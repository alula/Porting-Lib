...
public class ItemHandlerStorage implements Storage<ItemVariant> {
	...
	public long insert(ItemVariant resource, long maxAmount, TransactionContext transaction) {
		...
		ItemStack remainder = ItemHandlerHelperForge.insertItemStacked(handler, toInsert, true);
		transaction.addCloseCallback((t, result) -> {
			if (result.wasCommitted()) {
				ItemHandlerHelperForge.insertItemStacked(handler, toInsert, false);
			}
		});
	...
	public long extract(ItemVariant resource, long maxAmount, TransactionContext transaction) {
		...
		ItemStack extracted = ItemHandlerHelperForge.extract(handler, toExtract, true);
		transaction.addCloseCallback((t, result) -> {
			if (result.wasCommitted()) {
				ItemHandlerHelperForge.extract(handler, toExtract, false);
			}
		});
